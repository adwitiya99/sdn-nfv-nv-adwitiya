...
class Rule:
    ...
    def set_values(self, values):
        if values is None or (
                not isinstance(values, dict) and not ("required" in self.name and isinstance(values, list))):
...
class accepted(Rule):
    ...
    def message(self):
        return " must be checked or True."
...
class digits_between(Rule):
    ...
    def passes(self, value):
        ...
        return isinstance(value, str) and value.isdigit() and len(str(value)) >= int(min) and len(str(value)) <= int(
            max)
...
class required(Rule):
    ...
    def message(self):
        return self.attribute + ' field is ' + self.name+"."
...
class required_if(Rule):
    ...
    def passes(self):
        ...
        for (field, value) in options:
            ...
            # type confusion because values provided in options are always string -> needs notice
