...
class Validator:
    ...
    def run_validation(self, values):
        ...
        for attribute, rules in self.rules.items():
            for rule in rules:
                ...
                if isinstance(rule, (
                        present, required_if, required_unless, required_with, required_with_all, required_without,
    ...
    def error_message(self, request, field=None):
        er = {key: "<br>".join(value) for key, value in self.errors.items()}
        if field is None:
            ...
            if field in er:
                messages.error(request, er[field], extra_tags=field)
                return er[field]
            else:
                raise Exception("The is no such fields named {}.".format(field))
